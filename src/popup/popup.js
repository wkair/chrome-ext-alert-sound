document.addEventListener('DOMContentLoaded', () => {
    ...
    chrome.storage.local.get(['soundEnabled', 'whitelistEnabled', 'whitelist'], (data) => {
    ...
    whitelistToggle.addEventListener('change', () => {
        chrome.storage.local.set({ whitelistEnabled: whitelistToggle.checked }, () => {
        });
    });
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
        document.getElementById('whitelistInput').value = extractDomain(tabs[0].url);
    });
});
...
function addWhitelistElement(site) {
    ...
    li.textContent = site + "dddddddddddddddddddddddddddddddddddddddddd";
